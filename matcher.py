...
def highlight_single_field(pil_image, field_result):
    ...
    Draws a highlighted bounding box around ONLY that field.
    Returns an annotated PIL image (original is not modified).
    """
    ...
    # Pad the highlight area slightly for visibility
    pad = 4
    rx1, ry1 = max(0, x1 - pad), max(0, y1 - pad)
    rx2, ry2 = min(img_copy.width, x2 + pad), min(img_copy.height, y2 + pad)
    
    # Fill with semi-transparent bright green/yellow
    overlay_np[ry1:ry2, rx1:rx2] = [0, 255, 80, 60]
    overlay = Image.fromarray(overlay_np)
    img_copy = Image.alpha_composite(img_copy, overlay)

    # Convert to RGB for OpenCV drawing (border + label)
    cv_img = cv2.cvtColor(np.array(img_copy.convert("RGB")), cv2.COLOR_RGB2BGR)
    
    # Draw solid green border
    cv2.rectangle(cv_img, (rx1, ry1), (rx2, ry2), (0, 220, 0), 3)
    ...
    label_y = max(th + 10, ry1 - 8)
    
    # White background for readability
    cv2.rectangle(cv_img, (rx1, label_y - th - 6), (rx1 + tw + 8, label_y + 4), (255, 255, 255), -1)
    cv2.rectangle(cv_img, (rx1, label_y - th - 6), (rx1 + tw + 8, label_y + 4), (0, 220, 0), 2)
    cv2.putText(cv_img, label, (rx1 + 4, label_y - 2), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 200), thickness)
